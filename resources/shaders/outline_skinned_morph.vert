...
void main() {
    ...
    vec3 world_normal = normalize(mat3(model) * skinned_normal.xyz);
    vec4 world_pos = model * skinned_pos;
    
    v_normal = world_normal;
    v_uv = morphed_uv;
    
    vec3 offset = world_normal * outline_thickness;
    gl_Position = projection * view * (world_pos + vec4(offset, 0.0));
}
