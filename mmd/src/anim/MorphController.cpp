...
void MorphController::updateMorphOffsets()
{
    ...
    mMorphVbo->write(mPosOffsets.data(), mPosOffsets.size() * sizeof(float));
    if (mUvMorphVbo) {
        mUvMorphVbo->write(mUvOffsets.data(), mUvOffsets.size() * sizeof(float));
    }
}
