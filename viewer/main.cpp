...
int main(int argc, char* argv[])
{
    ...
    if (it != MODELS.end()) pmxPath = fs::u8path(it->second);
    else pmxPath = fs::u8path(modelName);
    fs::path vpdPath = fs::u8path("resources/vpd/自然站姿.vpd");
    fs::path projRoot = fs::weakly_canonical(fs::path(MMD_PROJECT_ROOT));
    if (pmxPath.is_relative()) pmxPath = projRoot / pmxPath;
    ...
    app.onMouseButton = [&cam](int b, int a, int) { if (b == GLFW_MOUSE_BUTTON_LEFT) cam.onMouseButton(a == GLFW_PRESS); };
    app.onCursorPos  = [&cam](double x, double y) { cam.onCursorPos(x, y); };
    app.onScroll     = [&cam](double, double yo) { cam.onScroll(yo); };
    app.onKey = [&](int key, int, int act, int) {
        ...
        if (key == GLFW_KEY_H && act == GLFW_PRESS) model.showModel(!model.showModel());
        if (key == GLFW_KEY_O && act == GLFW_PRESS) model.showOutline(!model.showOutline());
        if (key == GLFW_KEY_T && act == GLFW_PRESS) model.showToon(!model.showToon());
        ...
        if (key == GLFW_KEY_R && act == GLFW_PRESS) cam.reset();
        ...
        if (key == GLFW_KEY_SPACE && act == GLFW_PRESS) {
            if (model.hasVmd()) model.vmdPlaying() ? model.vmdPause() : model.vmdPlay();
        }
        if (key == GLFW_KEY_L && act == GLFW_PRESS) {
            if (model.hasVmd()) model.setVmdLoop(!model.vmdLoop());
        }
        if (key == GLFW_KEY_LEFT_BRACKET && act != GLFW_RELEASE) {
            if (model.hasVmd()) model.setVmdFrame(std::max(0.0f, model.vmdCurrentFrame() - 30));
        }
        if (key == GLFW_KEY_RIGHT_BRACKET && act != GLFW_RELEASE) {
            if (model.hasVmd()) model.setVmdFrame(std::min(model.vmdMaxFrame(), model.vmdCurrentFrame() + 30));
        }
    };
    ...
    app.onUpdate = [&](float dt) {
        ...
        fc++; et += dt;
        if (et >= 0.5f) {
            app.setTitle("MMD PMX Viewer - " + model.modelName() + " [" + std::to_string((int)(fc/et)) + " FPS]");
            fc = 0; et = 0;
        }
    };
